...
@section('backstageContent')
    ...
    <form action="{{ route('backstage.concerts.update', $concert) }}" class="bg-soft p-xs-y-5" method="POST">
        {{ csrf_field() }}
        {{ method_field('PATCH') }}
            ...
            <div class="container m-xs-b-4">
                <div class="alert alert-danger">
                    <h2 class="text-base text-danger wt-bold m-xs-b-2">
                        There {{ $errors->count() === 1 ? 'is' : 'are' }} {{ $errors->count() }} {{ Str::plural('error', $errors->count()) }} with this concert:
                    </h2>
                        ...
                        <div class="card">
                            <div class="card-section">
                                <div class="form-group {{ $errors->first('title', 'has-error') }}">
                                    ...
                                    <input type="text" class="form-control" name="title" value="{{ old('title', $concert->title) }}" placeholder="The Headliners">
                                </div>
                                <div class="form-group {{ $errors->first('subtitle', 'has-error') }}">
                                    <label for="subtitle" class="form-label">Subtitle</label>
                                    <input type="text" class="form-control" name="subtitle" value="{{ old('subtitle', $concert->subtitle) }}" placeholder="with The Openers (optional)">
                                </div>
                                    ...
                                    <div class="col col-md-6">
                                        <div class="form-group {{ $errors->first('ticket_price', 'has-error') }}">
                                            ...
                                            <span class="input-group-addon text-dark-muted">
                                                ...
                                                <input name="ticket_price" class="form-control" placeholder="0.00" value="{{ old('ticket_price', $concert->formatted_ticket_price) }}">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col col-md-6">
